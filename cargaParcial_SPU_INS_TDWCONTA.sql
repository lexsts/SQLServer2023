...
set @dt_base_processamento='20220414'
set @p_data_inicio='20220414'
set @p_data_fim='20220414'
...
--	FIM TRECHO PARA TESTES */
		...
		SET @p_data_inicio		= GETDATE();
			...
			OPTION(RECOMPILE,MAXDOP 16) --HINT DE MELHORIA
		...
		/*MELHORIA PARA EVITAR KEY LOOKUP
		CREATE NONCLUSTERED INDEX IE03_TDWICDX_CONTA
		ON hst.TDWICDX_CONTA (num_sequencial_tipo_conta)
		INCLUDE (num_sequencial_conta,data_inicio_vigencia_historico,ind_conta_propria_participante_negociacao,ind_cadastro_simplificado,dthr_inclusao,dthr_alteracao,num_identificacao_conta,num_sequencial_situacao,
		cod_motivo_situacao,data_situacao_atual,cod_titular_conta,cod_participante_negociacao,num_sequencial_titular_conta_endereco,num_sequencial_titular_conta_telefone,data_fim_vigencia_historico,
		num_sequencial_titular_conta_email,cod_operacional_participante)
		*/
		...
		from hst.TDWCLS_CALENDARIO caled (nolock) 
	   inner join hst.TDWICDX_CONTA conta with(nolock,index(IE03_TDWICDX_CONTA)) --HINT PARA FORÇAR O INDICE
	   ...
	   WHERE caled.data_calendario = @dt_base_processamento
		...
		/*MELHORIA PARA EVITAR KEY LOOKUP
		CREATE NONCLUSTERED INDEX IE03_TDWICDX_ENTIDADE
		ON hst.TDWICDX_ENTIDADE (num_sequencial_entidade,data_inicio_vigencia_historico)
		INCLUDE (data_fim_vigencia_historico,cod_tipo_pessoa,num_sequencial_tipo_entidade)
		*/
		...
		INNER JOIN  hst.TDWICDX_ENTIDADE as ent WITH(nolock,INDEX(IE03_TDWICDX_ENTIDADE)) --HINT PARA FORÇAR O INDICE
		...
		/*MELHORIA PARA EVITAR KEY LOOKUP
		CREATE NONCLUSTERED INDEX IE03_TDWICDX_PESSOA_FISICA
		ON hst.TDWICDX_PESSOA_FISICA (num_sequencial_entidade,data_inicio_vigencia_historico)
		INCLUDE (nome_completo,cod_ocupacao,data_fim_vigencia_historico)
		*/
		 SELECT convert(smalldatetime,caled.data_calendario) data_calendario
		 ...
		 FROM hst.TDWCLS_CALENDARIO caled
		INNER JOIN  hst.TDWICDX_PESSOA_FISICA as pf WITH(nolock,INDEX(IE03_TDWICDX_PESSOA_FISICA)) --HINT PARA FORÇAR O INDICE
		...
		/*MELHORIA PARA EVITAR KEY LOOKUP
		CREATE NONCLUSTERED INDEX IE03_TDWICDX_PESSOA_JURIDICA
		ON hst.TDWICDX_PESSOA_JURIDICA (num_sequencial_entidade,data_inicio_vigencia_historico)
		INCLUDE (nome_razao_social,cod_atividade_economica_primaria,data_fim_vigencia_historico)
		*/
		 ...
		 FROM hst.TDWCLS_CALENDARIO caled
		INNER JOIN  hst.TDWICDX_PESSOA_JURIDICA as pj WITH(nolock,INDEX(IE03_TDWICDX_PESSOA_JURIDICA)) 
		...
		/*MELHORIA PARA EVITAR KEY LOOKUP
		CREATE NONCLUSTERED INDEX IE03_TDWICDX_TITULAR_CONTA
		ON hst.TDWICDX_TITULAR_CONTA (cod_titular_conta,cod_participante_negociacao,data_inicio_vigencia_historico)
		INCLUDE (cod_tipo_pessoa,data_fim_vigencia_historico,ind_pessoa_vinculado)
		*/
		 SELECT convert(smalldatetime,caled.data_calendario) data_calendario
		 ...
		 FROM hst.TDWCLS_CALENDARIO caled
		INNER JOIN  hst.TDWICDX_TITULAR_CONTA as tit WITH(nolock,INDEX(IE03_TDWICDX_TITULAR_CONTA))
		...
		/*MELHORIA PARA EVITAR KEY LOOKUP
		CREATE NONCLUSTERED INDEX IE03_TDWICDX_TITULAR_CONTA_PESSOA_FISICA
		ON hst.TDWICDX_TITULAR_CONTA_PESSOA_FISICA (cod_titular_conta,cod_participante_negociacao,data_inicio_vigencia_historico)
		INCLUDE (cod_ocupacao,data_fim_vigencia_historico)
		*/
		 SELECT convert(smalldatetime,caled.data_calendario) data_calendario
		 ...
		 FROM hst.TDWCLS_CALENDARIO caled
		INNER JOIN  hst.TDWICDX_TITULAR_CONTA_PESSOA_FISICA as tit_conta_pf with(nolock,index(IE03_TDWICDX_TITULAR_CONTA_PESSOA_FISICA)) 
		...
		CREATE CLUSTERED INDEX IE_ADWCONTA ON DW.#ADWCONTA (num_sequencial_conta) WITH ( DATA_COMPRESSION = PAGE, FILLFACTOR=40)
		CREATE INDEX IE_ADWCONTA2 ON DW.#ADWCONTA (cod_operacional_participante) WITH ( DATA_COMPRESSION = PAGE, FILLFACTOR=40)
		...
		--;with xcte as (
		 ...
		 update dw.#ADWCONTA set 
                                num_sequencial_conta_final = num_sequencial_conta 
                                ,num_identificacao_conta_final = num_identificacao_conta
                               ,num_sequencial_entidade_participante_final = num_sequencial_entidade_participante
                               ,num_sequencial_entidade_investidor_final = num_sequencial_entidade_investidor
                               ,cod_operacional_participante_final = cod_operacional_participante
                               ,num_documento_identificacao_final = num_documento_identificacao
                               ,num_documento_identificacao_formatado_final = num_documento_identificacao_formatado
                               ,num_documento_identificacao_cvm_final = num_documento_identificacao_cvm
                               ,nome_completo_razao_social_final = nome_completo_razao_social
                               ,cod_tipo_pessoa_final = cod_tipo_pessoa
                               ,cod_atividade_economica_primaria_final = cod_atividade_economica_primaria
                               ,cod_ocupacao_final = cod_ocupacao
                               ,num_sequencial_situacao_final = num_sequencial_situacao
                               ,num_sequencial_situacao_vinculo_final = num_sequencial_situacao_vinculo_final
                               ,num_documento_nif_final = num_documento_nif
         ,num_sequencial_tipo_entidade_investidor_final = num_sequencial_tipo_entidade_investidor

                               where dw.#ADWCONTA.num_sequencial_conta_final is null
                                               and dw.#ADWCONTA.ind_conta_final = 1            
