...
WHERE NAME NOT IN (SELECT NAME FROM SYS.DATABASES A WHERE STATE<>0)
...
SELECT DISTINCT A.*,
		'SAOCUAPPD75V ' + '75V_' + A.DATABASE_NAME +
			'_' + A.FILE_NAME + '_u ' + CAST(A.SPACE_USED_IN_MB AS VARCHAR) AS RESULT
FROM   #db_file_info A

UNION ALL
SELECT DISTINCT A.*,
		'SAOCUAPPD75V ' + '75V_' + A.DATABASE_NAME +
			'_' + A.FILE_NAME + '_t ' + CAST(A.FILE_SIZE_IN_MB AS VARCHAR) AS RESULT
FROM   #db_file_info A
